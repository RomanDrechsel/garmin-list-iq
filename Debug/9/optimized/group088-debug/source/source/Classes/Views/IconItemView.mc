...
module Views {
    class IconItemView extends ItemView {
        ...
        protected function validate(dc as Dc) as Void {
            var validate;
            validate = self._needValidation;
            ItemView.validate(dc);
            if (validate) {
                {
                    validate /*>i<*/ = 0;
                    for (; validate /*>i<*/ < self.Items.size(); validate /*>i<*/ += 1) {
                        dc /*>icon<*/ = self.Items[validate /*>i<*/].getIcon();
                        if (dc /*>icon<*/ != null) {
                            self.Items[validate /*>i<*/].setIconInvert(dc /*>icon<*/ == self._itemIcon ? self._itemIconInvert : self._itemIconDoneInvert);
                        }
                    }
                }
            }
