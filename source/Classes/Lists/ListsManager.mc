...
module Lists
{
    ...
    typedef List as Dictionary<String, String or Array<Dictionary<String, String or Boolean or ListItemsItem>>>;
    typedef ListIndexItemType as Dictionary<String, String or Number>;
    typedef ListIndexType as Array<Dictionary<String, ListIndexItemType>>;
    ...
    class ListsManager
    {
        ...
        function addList(data as Application.PropertyValueType) as Boolean
        {
            ...
            for (var i = 0; i < listitems.size(); i++)
            {
                if (listitems[i].hasKey("item"))
                {
                    ...
                    else
                    {
                        ...
                        item.add(listitems[i]["item"]);
                        item.add(listitems[i]["note"]);
            ...
            {
                ToastUtil.Toast(Rez.Strings.EStorageFull, ToastUtil.ERROR);
                return false;
            }
            catch (e)
            {
                ToastUtil.Toast(Rez.Strings.EStorageError, ToastUtil.ERROR);
                return false;
            }
            ...
            ToastUtil.Toast(Rez.Strings.ListRec, ToastUtil.SUCCESS);
            return true;
        }
        ...
        function deleteList(uuid as String) as Void
        {
            ...
            {
                ToastUtil.Toast(Rez.Strings.ListDel, ToastUtil.SUCCESS);
            }
        }
        ...
        function clearAll() as Void
        {
            ...
            ToastUtil.Toast(Rez.Strings.StDelAllDone, ToastUtil.SUCCESS);
        }
            ...
            if (index != null && index.size() > 0)
            {
                ...
                for (var i = 0; i < index.keys().size(); i++)
                {
                    ...
                    if (dict != null && dict instanceof Dictionary)
                    {
                        ...
                        if (!dict.hasKey("key") || !(dict.get("key") instanceof String)
                        || !dict.hasKey("name") || !(dict.get("name") instanceof String))
                        {
                            delete.add(key);
                            continue;
                        }
