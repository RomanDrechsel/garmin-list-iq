...
module Views 
{
    class ListSettingsView extends CustomView
    {
        ...
        private enum { SETTINGS_MOVEDOWN, SETTINGS_DELETE }
        ...
        function onLayout(dc as Dc) as Void
        {
            ...
            self.Items.add(new ButtonViewItem(self.MainLayer.getDc(), Application.loadResource(Rez.Strings.StDelList), SETTINGS_DELETE, self._verticalPadding));
        }
        ...
        function onListTap(position as Number, item as ViewItem) as Void
        {
            if (item != null)
            {
                if (item.BoundObject == SETTINGS_MOVEDOWN)
                {
                    ...
                    {
                        item.setIcon(self.MainLayer, self._itemIconUncheck);
                    }
                    else
                    {
                        item.setIcon(self.MainLayer, self._itemIconCheck);
                    }
